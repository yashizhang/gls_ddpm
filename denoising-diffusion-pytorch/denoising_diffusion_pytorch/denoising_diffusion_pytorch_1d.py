...
class GaussianDiffusion1D(Module):
    def __init__(
        ...
        *,
        ...
        channels = None,
        self_condition = None,
        loss_method: Literal['mse', 'mmd', 'mmd-hessian', 'mmd-hessian-hvp'] = 'mse', 
        channel_first = True
    ):
        super().__init__()
        self.model = model
        ...
        self.samples_loss = SamplesLoss(loss='energy', blur=0.05)
        ...
        elif loss_method == 'mmd':
            def loss_fn(x, y, t):
                loss = self.samples_loss(x, y)
                return loss.mean()
            self.loss_fn = torch.compile(loss_fn)
        elif loss_method == 'mmd-hessian':
            def loss_fn(x, y, t):
                loss = lambda x: self.samples_loss(x, y)
                hess = hessian(loss, x)
                # compute ||x - y||_{hess} = (x-y)^T H (x-y)
                return ((x - y).T @ hess @ (x - y)).mean()
            self.loss_fn = torch.compile(loss_fn)
        elif loss_method == 'mmd-hessian-hvp':
            def loss_fn(x, y, t):
                diff = x - y
                def scalar_loss(inp):
                    return self.samples_loss(inp, y).sum()
                _, hv_prod = hvp(scalar_loss, x, diff)
                quad = (diff * hv_prod).reshape(diff.shape[0], -1).sum(dim=1)
                return quad.mean()
            self.loss_fn = torch.compile(loss_fn)
        else:
    ...
    def model_predictions(self, x, t, x_self_cond = None, clip_x_start = False, rederive_pred_noise = False, model_forward_kwargs: dict = dict()):
        ...
        model_output = self.model(x, t.unsqueeze(1).float(), **model_forward_kwargs)
    ...
    def p_mean_variance(self, x, t, x_self_cond = None, clip_denoised = False, model_forward_kwargs: dict = dict()):
    ...
    @torch.no_grad()
    def p_sample(self, x, t: int, x_self_cond = None, clip_denoised = False, model_forward_kwargs: dict = dict()):
    ...
    def p_losses(self, x_start, t, noise = None, model_forward_kwargs: dict = dict(), return_reduced_loss = True):
        ...
        model_out = self.model(x, t.float(), **model_forward_kwargs)
        ...
        #model_out = model_out.reshape(-1, 2000).contiguous()
        #target = target.reshape(-1, 2000).contiguous()
        '''
        if self.loss_method == "mse":
            ...
            return loss.mean() 
        elif self.loss_method == "mmd":
            samples_loss = SamplesLoss(loss='energy', blur=0.05)
            loss = samples_loss(model_out, target)
            return loss.mean()
        elif self.loss_method == "mmd-hessian":
            
            samples_loss = partial(SamplesLoss(loss='energy', blur=0.05), )

            samples_loss = partial(samples_loss, x=model_out, y=target)
            # 
        else:
            raise ValueError(f'unknown loss method {self.loss_method}')
        '''
        return self.loss_fn(model_out, target, t)

    def forward(self, img, *args, **kwargs):
        b, n, device, seq_length, = img.shape[0], img.shape[self.seq_index], img.device, self.seq_length

        assert n == seq_length, f'seq length must be {seq_length}'
        t = torch.randint(0, self.num_timesteps, (b,), device=device).long().unsqueeze(1) # (b, 1)
...
class Trainer1D(object):
    def __init__(
        ...
        *,
        ...
        dl = DataLoader(dataset, batch_size = train_batch_size, shuffle = True, pin_memory = True, num_workers = 8, prefetch_factor = 32, drop_last = True)
        ...
        def lr_lambda(step):
            ...
            warmup_steps = max(self.lr_warmup_steps, 0)

            if warmup_steps > 0 and step < warmup_steps:
                return (step + 1) / warmup_steps

            decay_steps = max(1, self.train_num_steps - warmup_steps)
            progress = 0.
        ...
        self.scheduler = torch.optim.lr_scheduler.LambdaLR(self.opt, lr_lambda = lr_lambda)
    ...
    def load(self, milestone):
        ...
        scheduler_state = data.get('scheduler') if isinstance(data, dict) else None
        if exists(self.scheduler) and exists(scheduler_state):
            ...
            while self.step < self.train_num_steps:
                ...
                if accelerator.is_main_process:
                    ...
                    if exists(self.wandb_run):
                        ...
                        elif exists(grad_norm):
                            metrics['train/grad_norm'] = float(grad_norm)
                        metrics['train/global_step'] = self.step
                        if exists(self.scheduler):
                            progress_denominator = max(self.lr_warmup_steps, 1)
                            warmup_progress = 1. if self.lr_warmup_steps <= 0 else min(self.step / progress_denominator, 1.)
                            metrics['train/lr_warmup_progress'] = warmup_progress
